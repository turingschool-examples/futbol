...
class LeagueStats < Futbol
  ...
  def best_offense
    ...
    @games.each do |game|
      best_offense[filter_by_team_id(game.away_team_id)] = average_number_of_goals(game)
    end
  ...
  def average_number_of_goals(game)
    total_goals = (total_goals_by_team(game.away_team_id) + total_goals_by_team(game.home_team_id))
