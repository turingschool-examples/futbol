...
class StatTracker
  ...
  def most_accurate_team(season)
    season_games = @season_manager.game_id_by_season(season)
    team_id = @game_team_manager.most_accurate_team(season_games)
    @team_manager.team_name(team_id)
  end

  def least_accurate_team(season)
    season_games = @season_manager.game_id_by_season(season)
    team_id = @game_team_manager.least_accurate_team(season_games)
    @team_manager.team_name(team_id)
  end

  def most_tackles(season)
    season_games = @season_manager.game_id_by_season(season)
    team_id = @game_team_manager.most_tackles(season_games)
    @team_manager.team_name(team_id)
  end

  def fewest_tackles(season)
    season_games = @season_manager.game_id_by_season(season)
    team_id = @game_team_manager.fewest_tackles(season_games)
    @team_manager.team_name(team_id)
  end
