...
class GameTeams
  ...
  def self.create_game_teams_data_objects(path)
    ...
    game_team_objects << GameTeams.new(row["game_id"], row["team_id"].to_i, row["HoA"].to_sym, row["result"].to_sym, row["settled_in"].to_sym, row["head_coach"], row["goals"].to_i, row["shots"].to_i, row["tackles"].to_i, row["pim"].to_i, row["powerPlayOpportunities"].to_i, row["powerPlayGoals"].to_i, row["faceOffWinPercentage"].to_f, row["giveaways"].to_i, row["takeaways"].to_i)
    end
    game_team_objects
  end
